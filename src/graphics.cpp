...
bool graphic_class::update(game_class *game)
{
    bool quit = false;
    SDL_Event e;
    ...
    int x = 0, y = 0;
    for (auto row : *game->map_get()->tiles_map_get())
    {
        for (auto hex : *row)
        {
    ...
    for (auto& u_list : list_of_u_list)
    {
        for (auto& u : u_list)
        {
            if (u->hp_get() > 0)
            {
                ...
                tile = game->map_get()->tile_get(u->q_get(), u->r_get());
    ...
    */
    ...
    {
        hex_tile* tile = nullptr;
        tile = game->map_get()->tile_get(food->q_get(), food->r_get());
    ...
    for (const auto base : *base_list)
    {
        ...
        hex_tile* tile = nullptr;
        tile = game->map_get()->tile_get(base->q_get(), base->r_get());
